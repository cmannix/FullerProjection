...
namespace FullerProjection.Projection
{
    public static class IcosahedronConstants
    {
        ...
        public static Dictionary<int, Cartesian> CentreCoordinates = new Dictionary<int, Cartesian>();

        static IcosahedronConstants()
        {
            IcosahedronVertices = new Dictionary<Axis, double[]>
            {
                {Axis.X, IcosahedronVerticesX },
                {Axis.Y, IcosahedronVerticesY },
                {Axis.Z, IcosahedronVerticesZ }
            };
        ...
        public static Dictionary<Axis, double[]> IcosahedronVertices;

        private static Cartesian _GetCentreCoordinate(int index)
        {
            ...
            {
                hold_x += IcosahedronVertices[Axis.X][idx];
                hold_y += IcosahedronVertices[Axis.Y][idx];
                hold_z += IcosahedronVertices[Axis.Z][idx];
            }
            ...
            var magnitude = Sqrt(Pow(hold_x, 2) + Pow(hold_y, 2) + Pow(hold_z, 2));
            hold_x /= magnitude;
            hold_y /= magnitude;
            hold_z /= magnitude;


            return new Cartesian(hold_x, hold_y, hold_z);
        }
